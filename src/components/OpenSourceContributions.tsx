...
const SEARCH_QUERIES: Record<FilterType, string> = {
  merged: "author:RAWx18 type:pr is:merged",
  open: "author:RAWx18 type:pr is:open",
  closed: "author:RAWx18 type:pr is:closed is:unmerged",
};
    ...
    try {
      ...
      if (data.data?.search?.edges) {
        const fetchedPRs = data.data.search.edges
          ...
          .filter((pr: PR) => !(pr.title === "Main" && pr.repository.nameWithOwner === "RAWx18/flexprice-storybook"));
