...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${project.title} · Ryan Madhuwala`,
      description: project.description,
      url: `/projects/${slug}`,
      images: [{ url: project.src, alt: `${project.title} preview` }],
    },
    twitter: {
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {project.tech.map((t: TechItem, i: number) => {
              ...
              const img = isKey ? undefined : t.img;
              ...
              return (
                <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-900 border border-black/10 dark:border-white/5 rounded-md text-[12px] font-medium text-zinc-700 dark:text-zinc-300">
                  ...
                  {img && <Image src={img} alt={label} width={14} height={14} className="w-3.5 h-3.5 object-contain grayscale dark:invert" />}
                  <span>{label}</span>
                </div>
              );
            })}
